...
SELECT 
  ...
  SPLIT(FORMAT_TIMESTAMP('%Y-%m-%dT%H:%M:%E*S', TIMESTAMP(data_hora)), 'T')[OFFSET(0)] AS data,
  SPLIT(FORMAT_TIMESTAMP('%Y-%m-%dT%H:%M:%E*S', TIMESTAMP(data_hora)), 'T')[OFFSET(1)] AS horario,
